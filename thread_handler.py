def main():
    ...
    if hmsysteme.check_ifdebug():
        ...
        t1 = multiprocessing.Process(
            target=led_calibration_temp.led_calibration_temp,
            args=("led_calibration_temp", tempqueue),
        )
        t1.start()
        print("led calibration process started")

    t2 = multiprocessing.Process(
        target=hardware_com.hardware_com,
        args=("Hardware_com", path, queue, queue4, prequeue, warmupqueue, size),
    )
    t2.start()
    print("hardware_com process started")

    t3 = multiprocessing.Process(
        target=startscreen.startscreen, args=("startscreen", warmupqueue, activequeue)
    )
    ...
    while True:
        ...
        if str(hwqueue.get()) == "off":
            t1.terminate()
            t1 = multiprocessing.Process(
                target=hardware_com.hardware_com, args=("Hardware_com", path, queue, queue4, size)
            )
            time.sleep(0.5)
            t1.start()
