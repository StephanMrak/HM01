...
class SuperImage(gui.Image):
    def __init__(self, file_path_name: str | Path = None, **kwargs):
...
class MyApp(App):
    def __init__(self, *args):
        ...
        try:
            ...
            RPi.GPIO.output(4, 0)
            self.GPIO_exessible = True
        except Exception as e:
            self._log.error(f"GPIO initialization failed: {e}")
    ...
    def idle(self):
        ...
        if time.time() - getattr(self, "_last_check", 0) > 1:  # check every second
    ...
    def start_game(self, widget):
        ...
        if self.P_game.is_alive():
            ...
            self._log.info(self.shared_ns.active_players)
            self._log.info(self.shared_ns.all_players)
    ...
    def page_settings(self):
        ...
        btn_close_current_game = gui.Button(
            "Close current game",
            width="31%",
            height="50px",
            margin="1%",
        ...
        btn_close_current_game.onclick.do(self.onclick_btn_close_current_game)
        vBox.append(btn_close_current_game)
        ...
        btn_system_shutdown.onclick.do(self.onclick_btn_system_shutdown)
        vBox.append(btn_system_shutdown)

        btn_kill_current_game = gui.Button(
            "Kill current game",
            width="31%",
            height="50px",
            margin="1%",
        ...
        btn_kill_current_game.onclick.do(self.onclick_btn_kill_current_game)
        vBox.append(btn_kill_current_game)
        ...
        btn_system_reset.set_enabled(self.GPIO_exessible)
        vBox.append(btn_system_reset)

        return vBox

    def onclick_btn_close_current_game(self, widget):
        if self.P_game.is_alive():
            self.shared_ns.active = False  # Ask nicely
            self.lbl_info_home.set_text("No game running")

    def onclick_btn_kill_current_game(self, widget):
        if self.P_game.is_alive():
            self.P_game.terminate()  # RAMPAGE
            self.lbl_info_home.set_text("No game running")

    def onclick_btn_stop_server(self, widget):
        # TODO try to set the Namespace.active variable false first and wait a second?
        self.onclick_btn_close_current_game(None)
        ii = 0
        while self.P_game.is_alive() and ii <= 20:
            if time.time() - getattr(self, "_last_check", 0) > 0.1:  # check every 0.1 seconds
                self._last_check = time.time()
                ii += 1
        self.onclick_btn_kill_current_game(None)
        self.close()

    def onclick_btn_system_shutdown(self, widget):
    ...
    def shut_down_computer_now(self, widget):
        ...
        if os.name == "nt":
            ...
            self._log.info("Unsupported operating system.")
    ...
    def set_preset(self, percentage="0"):
        # TODO kein plan wofür das hier ist
        # preq.put("0")
        try:
            RPi.GPIO.output(4, 1)
            time.sleep(0.1)
