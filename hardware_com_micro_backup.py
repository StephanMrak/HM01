...
def start(plot=False, process = True, debug = False, filename = "", threshold = 200, mode = 1):
    ...
    positionEstimator = PositionEstimator(.370, .215, mode)  # Initialize with width and height

    # Pass the plot and process flags to the receiver proces
    print(f"Starting receiver in: {mode}")
    print(f"Starting receiver with threshold: {threshold}")
    receiver = multiprocessing.Process(target=receiver_process, args=(plot_queue, data_queue, stop_flag, plot, process, threshold, mode))
    processor = multiprocessing.Process(target=data_processing, args=(data_queue, stop_flag, debug, process, positionEstimator))
...
def main():
    ...
    start(args.p, args.r, args.d, args.f, args.t, args.m)
