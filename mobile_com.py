...
def mobile_com(
    ...
    container_players_added = gui.VBox(
        width=width,
        height=height,
    ...
    def fill_grid_with_playernames():
        ...
        txt = gui.TextInput(
            width="30%", height="35px", style={"font-size": "30px", "text-align": "left"}
        )
        ...
        txtblank = gui.Label(
            "", width="50%", height="35px", style={"font-size": "25px", "text-align": "left"}
        )
        ...
        for i in range(0, len(playernames)):
            ...
            grid.append(
                {
                    "delete" + str(i + 1): deletea[i],
                    "label" + str(i + 1): lbl,
                    "check" + str(i + 1): checka[i],
                }
            )
            grid.set_row_gap(20)
    ...
    class PILImageViewverWidget(gui.Image):
        def __init__(self, pil_image=None, **kwargs):
            super(PILImageViewverWidget, self).__init__(
                os.path.join(path, "screencapture.jpg"), **kwargs
            )
    ...
    class HMInterface(App):
        ...
        def on_button_pressed5(self, widget):
            self.dialog = gui.GenericDialog(
                width=350, title="Reboot", message="Do you really want to reboot the system"
            )
